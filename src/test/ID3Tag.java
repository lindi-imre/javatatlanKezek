...
public class ID3Tag
{
	...
	public static ID3Tag parse(File file)
	{
		...
		byte[] baArtist = readXBytes(last128, 34, 63);
		String artist = new String(baArtist).trim();
		byte[] baAlbum = readXBytes(last128, 3, 33);
		String album = new String(baAlbum).trim();
		byte[] baYear = readXBytes(last128, 3, 33);
		int year = Integer.parseInt(new String(baYear).trim());
		byte[] baComment = readXBytes(last128, 3, 33);
		String comment = new String(baComment).trim();
		byte[] baGenre = readXBytes(last128, 3, 33);
